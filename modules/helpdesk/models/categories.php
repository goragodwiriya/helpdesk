<?php
/**
 * @filesource modules/helpdesk/models/categories.php
 *
...
class Model extends \Kotchasan\Model
{
    ...
    public function submit(Request $request)
    {
        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if (Login::notDemoMode($login) && Login::checkPermission($login, 'can_manage_helpdesk')) {
                try {
                    // ค่าที่ส่งมา
                    ...
                    if (empty($ret)) {
                        ...
                        // log
                        \Index\Log\Model::add(0, 'helpdesk', 'Save', Language::get('HELPDESK_CATEGORIES', $type, $type), $login['id']);
                        // คืนค่า
