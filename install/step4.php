    ...
    if (!$error) {
        ...
        if (!$error) {
            try {
                ...
                $sql .= "(3, 'agent1', '$salt', '".sha1($password_key.'agent1'.$salt)."', NULL, 3, ',helpdesk_agent,', 'Agent 1', '$today');";
                $sql .= "(4, 'agent2', '$salt', '".sha1($password_key.'agent2'.$salt)."', NULL, 3, ',helpdesk_agent,', 'Agent 2', '$today');";
                $db->query($sql);
            } catch (\PDOException $ex) {
                $error = true;
                $content[] = '<li class="incorrect">'.$ex->getMessage().'</li>';
